...
| G-02 | ActionExecutor لا يملك كل transports مركزيًا | كل transport الفعلي صار مسجلًا في `docs/direct_io_inventory.json` و`docs/DIRECT_IO_INVENTORY.md`: HTTP sync/async، Playwright، raw TCP/DNS، subprocess، مع توثيق API/GraphQL/upload/OOB كـlogical HTTP transports. اختبار G-02 يطابق source AST مع artifact ويمنع unclassified أو غير allowlisted sites. | G2 | **CLOSED** | qualification الحية لكل capability تظل منفصلة؛ لا confirmation بدون verifier/ProofBundle |
...
| G-08 | direct I/O inventory غير محكوم بCI allowlist | `scripts/scan_direct_io.py` يولد artifact machine-readable، و`tests/test_g02_direct_io_inventory.py` يطابقه مع source AST، و`APPROVED_DIRECT_FILES` يحكم raw imports/calls مع logical transport contracts. | G2 | **CLOSED** | أي transport جديد يتطلب إعادة توليد artifact ومراجعة allowlist؛ qualification التشغيلية لا تُستنتج من static inventory وحده |
...
3. جعل ProofBundle/promotion gate مركزيًا، ثم إكمال direct-I/O inventory — **تم إغلاق G-02 وG-08 بالـAST artifact وCI contract**.
